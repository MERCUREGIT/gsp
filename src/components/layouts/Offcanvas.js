import React, { Fragment } from 'react'
import { Link } from 'react-router-dom'

import logo from '../../assets/img/logo-3.png'



export default () => {

    return (
        <Fragment>
            {/* About Widget */}
            <div className="widget about-widget">
                <div className="logo mb-30">
                    <img src={logo} alt="" />
                </div>
                <p>
                <strong> GLOBAL SERVICES AND PRINTING </strong> is a multidimensional company that wields great competence and expertise in Printing and Radio Frequency Engineering Consultancy. We run an efficient printing press services adapted to technological advancement and staffed by seasoned and competent men and women.
      </p>
            </div>
            {/* Contact information Widget */}
            <div className="widget contact-info-widget">
                <h5 className="widget-title">Contact Us</h5>
                <ul>
                    <li>
                        <i className="far fa-map-marked-alt" />
          6701 Democracy Blvd, Suite 300, USA
        </li>
                    <li>
                        <i className="far fa-phone" />
          0800-123456 (24/7 Support Line)
        </li>
                    <li>
                        <i className="far fa-envelope-open" />
          info@example.com
        </li>
                </ul>
            </div>
            {/* tweets Widget */}
            <div className="widget tweets-widget">
                <h5 className="widget-title">Recent tweets</h5>
                <ul>
                    <li>
                        <Link to="#">
                            <i className="fab fa-twitter" />
                            <span>@username:</span> Sedut perspiciatis unde omnis iste natus error sit
            voluptatem accusantium...
          </Link>
                    </li>
                    <li>
                        <Link to="#">
                            <i className="fab fa-twitter" />
                            <span>@username:</span> Sedut perspiciatis unde omnis iste natus error sit
            voluptatem accusantium...
          </Link>
                    </li>
                    <li>
                        <Link to="#">
                            <i className="fab fa-twitter" />
                            <span>@username:</span> Sedut perspiciatis unde omnis iste natus error sit
            voluptatem accusantium...
          </Link>
                    </li>
                </ul>
            </div>
            {/* Social Link */}
            <div className="widget social-link">
                <h5 className="widget-title">Follow us</h5>
                <ul>
                    <li><Link to="#"><i className="fab fa-facebook-f" /></Link></li>
                    <li><Link to="#"><i className="fab fa-twitter" /></Link></li>
                    <li><Link to="#"><i className="fab fa-behance" /></Link></li>
                    <li><Link to="#"><i className="fab fa-linkedin" /></Link></li>
                    <li><Link to="#"><i className="fab fa-google" /></Link></li>
                </ul>
            </div>
        </Fragment>
    )
}